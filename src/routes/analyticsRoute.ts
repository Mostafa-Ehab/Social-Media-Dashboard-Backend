...
export function analyticsRoute(analyticsController: IAnalyticsController, authMiddleware: IAuthMiddleware): express.Router {
    ...
    router.get("/p/:platformId", authMiddleware.loginRequired, analyticsController.getAnalytics);
    router.get("/stats/:platformId", authMiddleware.loginRequired, analyticsController.getStats);
